...
kubectl apply -f ${GITHUB_URL}deployments/common/ns-and-sa.yaml
kubectl apply -f ${GITHUB_URL}deployments/rbac/rbac.yaml
...
kubectl apply -f ${GITHUB_URL}deployments/common/nginx-config.yaml

kubectl apply -f ${GITHUB_URL}deployments/common/ingress-class.yaml

kubectl apply -f ${GITHUB_URL}config/crd/bases/k8s.nginx.org_virtualservers.yaml
kubectl apply -f ${GITHUB_URL}config/crd/bases/k8s.nginx.org_virtualserverroutes.yaml
kubectl apply -f ${GITHUB_URL}config/crd/bases/k8s.nginx.org_transportservers.yaml
kubectl apply -f ${GITHUB_URL}config/crd/bases/k8s.nginx.org_policies.yaml
kubectl apply -f ${GITHUB_URL}config/crd/bases/k8s.nginx.org_globalconfigurations.yaml

# deploy the ingress as a single controller pod
#kubectl apply -f ${GITHUB_URL}deployments/deployment/nginx-ingress.yaml
# deploy the ingress with a controller pod on each node 
kubectl apply -f ${GITHUB_URL}deployments/daemon-set/nginx-ingress.yaml

# per default l'ingress ascolta su tutti i nodi su una porta random 
#kubectl create -f ${GITHUB_URL}deployments/service/nodeport.yaml

# configura invece l'ingress per ascoltare su tutti i nodi sulle porte 31080 e 31443 
curl -s -L ${GITHUB_URL}deployments/service/nodeport.yaml \
        | sed 's/targetPort: 80$/&\n    nodePort: 31080/' \
