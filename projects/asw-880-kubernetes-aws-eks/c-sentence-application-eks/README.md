# Sentence per Amazon AWS EKS

Questo sottoprogetto contiene il codice di specifica per rilasciare l'applicazione **sentence** u un cluster Amazon AWS EKS. 
...
Per avviare l'applicazione, eseguire lo script `deploy-sentence.sh` 
(oppure lo script `deploy-sentence-multi.sh` per avviare più repliche dei servizi per le parole). 

Si può monitorare l'applicazione mediante lo script `inspect-sentence.sh`. 
